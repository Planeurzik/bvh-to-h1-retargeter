...
Simpler motion retargeting to the H1 humanoid.
"""
...
def main():
    ...
    smpl_mask = create_conn_tree(robot, h1_joint_retarget_indices)
    ...
    def generate_trajectory():
        ...
        Ts_world_root, joints = solve_retargeting(
            ...
            h1_joint_retarget_indices=h1_joint_retarget_indices,
            smpl_mask=smpl_mask,
    ...
    generate_trajectory()
    joints = joints.at[:, -1].set(0.0)

    joints = joints.at[:, -5].set(0.0)

    joints = joints.at[:, -6].set(0.0)
...
def solve_retargeting(
    ...
    h1_joint_retarget_indices: jnp.ndarray,
    ...
    var_smpl_joints_scale = SmplJointsScaleVarH1(jnp.zeros(timesteps))
    ...
    def retargeting_cost(
        ...
        var_smpl_joints_scale: SmplJointsScaleVarH1,
        ...
        robot_pos = T_world_link.translation()[jnp.array(h1_joint_retarget_indices)]
    ...
    def pc_alignment_cost(
        ...
        T_root_link = jaxlie.SE3(robot.forward_kinematics(cfg=robot_cfg))
        T_world_link = T_world_root @ T_root_link
        link_pos = T_world_link.translation()[h1_joint_retarget_indices]
