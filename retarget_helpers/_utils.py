...
# When loaded from `h1_description`s 19-dof model.
H1_LINK_NAMES = [
    "pelvis",
    "left_hip_yaw_link",
    "left_hip_roll_link",
...
def get_humanoid_retarget_indices() -> tuple[jnp.ndarray, jnp.ndarray]:
    ...
    for smpl_name, h1_name in [
        ...
        h1_joint_retarget_indices.append(H1_LINK_NAMES.index(h1_name))

    smpl_joint_retarget_indices = jnp.array(smpl_joint_retarget_indices_to_h1)
    h1_joint_retarget_indices = jnp.array(h1_joint_retarget_indices)
    return smpl_joint_retarget_indices, h1_joint_retarget_indices
